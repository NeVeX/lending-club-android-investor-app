...
import com.mark.lendingclub.invest.api.service.ApiAccountService;
...
import com.mark.lendingclub.invest.util.UIAnimation;
import com.mark.lendingclub.invest.util.UIUtil;
...
public class HomeActivity extends Activity implements ApiCallback<AccountVO> {
    ...
    private static final String RECEIVED_INTEREST_LABEL = "Interest";
    private static final String RECEIVED_PRINCIPAL_LABEL = "Principal";
    private static final String OUTSTANDING_PRINCIPAL_LABEL = "Outstanding";

    public HomeActivity()
    {
        xValues.add(ACCOUNT_LABEL);
        xValues.add(CASH_LABEL);
        xValues.add(RECEIVED_INTEREST_LABEL);
        xValues.add(RECEIVED_PRINCIPAL_LABEL);
        xValues.add(OUTSTANDING_PRINCIPAL_LABEL);
        chartLabelMap.put(ACCOUNT_LABEL, "Total Account Balance");
        chartLabelMap.put(CASH_LABEL, "Total Cash Balance");
        chartLabelMap.put(RECEIVED_INTEREST_LABEL, "Received Interest Amount");
        chartLabelMap.put(RECEIVED_INTEREST_LABEL, "Received Interest Amount");
        chartLabelMap.put(OUTSTANDING_PRINCIPAL_LABEL, "Outstanding Principal");
    }
    ...
    private void setData(BarChart mChart, AccountVO accountVO) {
        ...
        yValues.add(new BarEntry((float) accountVO.getAccountTotal(), 0));
        yValues.add(new BarEntry((float) accountVO.getCashBalance(), 1));
        yValues.add(new BarEntry((float) accountVO.getRecievedInterest(), 2));
        yValues.add(new BarEntry((float) accountVO.getRecievedPrincipal(), 3));
        yValues.add(new BarEntry((float) accountVO.getOutstandingPrincipal(), 4));
    ...
    public void onApiResponse(AccountVO data) {
        if ( data != null) {
            pendingQuickInvestmentAmount.setText(UIUtil.getMoneyString(data.getAccuredInterest()));
            pendingFolioInvestedAmount.setText(UIUtil.getMoneyString(data.getInFundingBalance()));
            outstandingPrincipalOnNotesAmount.setText(String.valueOf(data.getTotalNotes()));
            showAccountOverview();
        }
        else
        {
