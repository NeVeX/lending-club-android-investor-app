...
import com.mark.lendingclub.invest.api.service.ApiAuthorizationService;
...
import com.mark.lendingclub.invest.config.ConfigurationVO;
...
public class SignInActivity extends Activity implements ApiCallback<Boolean> {
    ...
    private static final String SHARED_PREFERENCES_INVESTOR_ID = "INVESTOR_ID";
    private static final String SHARED_PREFERENCES_API_KEY = "API_KEY";
...
//        passwordText.setText("");
        ...
        String investorId = settings.getString(SHARED_PREFERENCES_INVESTOR_ID, "");
        if (investorId != null && investorId.length() > 0 )
        {
            userNameText.setText(investorId);
        }
        else
        {
            userNameText.setText("");
        }
        String apiKey = settings.getString(SHARED_PREFERENCES_API_KEY, "");
        if (apiKey != null && apiKey.length() > 0 )
        {
            passwordText.setText(apiKey);
        }
        else
        {
            passwordText.setText("");
    ...
    public void onApiResponse(Boolean canLogIn) {
        ...
        if ( canLogIn )
        {
            ...
            editor.putString(SHARED_PREFERENCES_INVESTOR_ID, ApplicationState.getState().getCurrentUser().getUserName());
            editor.putString(SHARED_PREFERENCES_API_KEY, ApplicationState.getState().getCurrentUser().getLcKey());
            editor.apply();
